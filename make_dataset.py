...
def worker():
    while True:
        ...
        with open(os.devnull, 'w') as devnull:
            # maybe remove old simulation data
            try:
...
for i in tqdm(range(num_runs)):
    ...
    rand_y = np.random.uniform(
        0.0 + rand_radius + 0.01,
        0.41 - rand_radius - 0.01
    )
    ...
    ET.SubElement(paths, "dirname").text = "{}{}{}/".format(
        os.path.abspath('.'),
        "/data/simulation_data/runlog_",
        str(i).zfill(5),
    )
    ...
    sim_cmd = "{}{}.xml".format(
        "./cylinder2d/cylinder2d ./data/xml_runs/run_",
        str(i).zfill(5)
    )
    rm_cmd = "rm -r {}{}".format(
        os.path.abspath('.'),
