...
class VTK_data:
    def __init__(self, base_dir, train_test_split=.8):
        ...
        # train vs test split (numbers under this value are
        # in train, over in test)
        ...
        for i in range(batch_size):
            if train:
                ...
                sample = np.random.randint(
                    self.split_line, len(self.boundarys)
                )
...
dataset.load_data()
batch_boundary, batch_data = dataset.minibatch(
    batch_size=batch_size, batch_type="flow")
